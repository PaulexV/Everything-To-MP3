...
describe('AuthService', () => {
  ...
  describe('getAccessToken', () => {
    it('should throw UnauthorizedException if user not found', async () => {
      ...
      await expect(service.getAccessToken('username', 'password')).rejects.toThrow(UnauthorizedException);
    });

    it('should throw UnauthorizedException if password does not match', async () => {
      mockUsersService.findOne.mockResolvedValueOnce({ password: 'wrong' });
      await expect(service.getAccessToken('username', 'password')).rejects.toThrow(UnauthorizedException);
    });

    it('should return a JWT if user is found and password matches', async () => {
      mockUsersService.findOne.mockResolvedValueOnce({ username: 'username', password: 'password' });
